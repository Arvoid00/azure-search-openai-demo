...
## Seamless Agency: Developing locally
...
1. Clone this repo
1. Place .azure folder (from Slack) in the root directort of the clone
1. Select the correct environment to work in `demo-genai` (Arvid private subscription), or `pwn-genai` (to be deployed to PWN subscription) by using the command `azd env select <environment-name>` in the terminal.
1. Login to the Microsoft service by running `azd auth login`. This will open a browser page. (Make sure to open the localhost later in the same window/chrome profile)
...
If not the case, run `azd up` to do the full package, provision, deploy cycle.

Full list of AZD commands: (or run `azd` in terminal )
...
Additional information can be found in the detailed guide in this repo.

[End of Seamless Install Guide]
